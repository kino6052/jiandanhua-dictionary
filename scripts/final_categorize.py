...
remaining_atomic = sorted([w for w in data['atomicWords'] if w not in all_cuts])
allowed_chars = set()
for w in remaining_atomic:
    for ch in w:
        allowed_chars.add(ch)

# For each char, find which remaining atomic words contain it
char_words = {}
for ch in allowed_chars:
    char_words[ch] = [w for w in remaining_atomic if ch in w]

# Print for analysis: char, example words
print(f'TOTAL CHARS: {len(allowed_chars)}')
print(f'TOTAL WORDS: {len(remaining_atomic)}')
print()

for ch in sorted(allowed_chars):
    examples = char_words[ch][:4]
    print(f'{ch}\t{" ".join(examples)}')
