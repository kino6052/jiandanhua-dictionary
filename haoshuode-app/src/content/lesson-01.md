...
One thing pinyin can't hide, though, is tone -- and tone is not a decoration, it's part of the word. Say `mā` (mother), `má` (hemp), `mǎ` (horse), and `mà` (to scold) with the wrong pitch, and you haven't mispronounced a word -- you've said a different one.
...
3. **Quotes wall off the untranslatable.** Proper names and imitated sounds -- things that aren't really Hao-shuo-de vocabulary at all -- are kept inside quotation marks: `"Beijing"`, `jiào "wāng-wāng"` (goes "woof woof"). If you see quotes, don't look for the word in the dictionary; it isn't one.
